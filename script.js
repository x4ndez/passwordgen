...
passwordRangeSet(8, 128); // Sets default password length
...
// Obtained from www.bestwordlist.com
...
let wordArray = unformattedString.split(" "); // Splits word list string into array
...
function createRandomPassword(len, bigArray) {
  ...
  if (password.criteria[2]) { // If word-by-word is selected...

    for (let i = 0; i < len; i++) {

      passwordArray[i] = bigArray[randomNumber(bigArray)] + "-"; // Create a password array from the word list, adding '-' to each word.

    }

    passwordString = passwordArray.join(""); // Convert password array to string
    passwordString = passwordString.slice(0, passwordString.length - 1); // Remove the "-" from the last word.

  } else { // If Alphanumeric or funky are selected...

    for (let i = 0; i < len; i++) {

      passwordArray[i] = bigArray[randomNumber(bigArray)]; // Create a password array from the alphanumeric/funky array.
...
function writePassword() {
  ...
  const bigArray = generateArray(password.criteria[0], password.criteria[1], password.criteria[2]); // Generates array according to the selected radio button

  password.value = createRandomPassword(password.length, bigArray); // Creates the password
